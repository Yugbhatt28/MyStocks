 ...
 * Data Structures:
 *   - IndexStack         — Custom stack for indices
 *   - IndexDeque         — Custom deque for indices
 *   - MinHeap            — Min-heap for price tracking
 *   - MaxHeap            — Max-heap for price tracking
 * Performance: Optimized for real-time stock analytics with minimal memory overhead.
 */
...
struct MinHeap
{
    ...
    void pop()
    {
        ...
        while (true)
        {
            ...
            if (left < size && prices[indices[left]] < prices[indices[smallest]])
                smallest = left;
            if (right < size && prices[indices[right]] < prices[indices[smallest]])
                smallest = right;
            if (smallest != i)
            {
                swap_idx(indices[i], indices[smallest]);
                i = smallest;
            }
            else
                break;
...
extern "C"
{
    ...
    double calculateVolatility(const double *prices, int n)
    {
        ...
        return roundCents(sqrt(variance / n));
    }
    ...
    {
        sliding_window_result[0] = 0;
        sliding_window_result[1] = 0;
        sliding_window_result[2] = 0;
        if (n <= 0 || windowSize <= 0)
            return sliding_window_result;
        int w = min(windowSize, n);
    ...
    {
        heap_profit_result[0] = 0;
        heap_profit_result[1] = 0;
        heap_profit_result[2] = 0;
        if (n < 2)
